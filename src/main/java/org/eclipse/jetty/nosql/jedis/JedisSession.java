...
/**
 * A bridge between the format stored ({@link Map}) and the {@link NoSqlSession}, that the {@link NoSqlSessionManager}
 * uses. For inspiration, the {@link MongoSessionManager} was used
 *
 * @author Friso Vrolijken Vrolijken
 * @since 19 sep. 2016
 */
class JedisSession {
    ...
    enum MapKey {
        ...
        /**
         * Version number, long starting with 1L, required
         */
        VERSION,
    ...
    private JedisSession(NoSqlSessionManager manager, Map<byte[], byte[]> fromDataStore) {
        ...
        if (getBoolean(MapKey.VALID)) {
            this.delegate = new NoSqlSession(manager, getLong(MapKey.CREATED), getLong(MapKey.ACCESSED),
                    getString(MapKey.ID), Long.valueOf(getString(MapKey.VERSION)));
     ...
     */
    static NoSqlSession asNoSqlSession(JedisSessionManager jedisSessionManager,
            Map<byte[], byte[]> complete) {
        if (complete == null) {
            return null;
        }
        // else
    ...
    /**
     * Helper method for transforming byte[] to boolean. The bytes must be stored in the persistentValue or else an
     * {@link IllegalStateException} will be thrown.
     * 
     * @param key
     *            the key by which the required value is known in the persistent value
     * @return true iff the persistentVvalue has a value for the key, that value is a byte[] of length 1 and the value
     *         is 1
     */
    ...
    private void setBoolean(MapKey key, boolean bool) {
        byte[] value = bool ? new byte[] { 1 } : new byte[] { 0 };
        persistentValue.put(key.getKey(), value);
    }

    /**
     * Helper method for transforming byte[] to long. The bytes must be stored in the persistentValue or else an
     * {@link IllegalStateException} will be thrown.
     * 
     * @param key
     *            the key by which the required value is known in the persistent value
    ...
    private long getLong(MapKey key) {
        ...
        if (value.length != Long.BYTES) {
            throw new IllegalStateException("An incorrect value was stored for " + key + " got " + value.length
                    + " bytes but expected " + Long.BYTES);
        }
        long result = 0;
    ...
    /**
     * Helper method for transforming long to byte[]. The bytes are subsequently stored in the persistentValue. If that
     * is not initialised, an {@link IllegalStateException} will be thrown.
     * 
     * @param key
     *            the key by which the required value is known in the persistent value
     * @param value
    ...
    /**
     * Helper method for transforming byte[] to String. The bytes must be stored in the persistentValue or else an
     * {@link IllegalStateException} will be thrown.
     * 
     * @param key
     *            the key by which the required value is known in the persistent value
    ...
    /**
     * Helper method to transform the attributes which should be stored as a {@link HashMap}. Not using the interface
     * Map here since that is not {@link Serializable} and {@link HashMap} is.
     * 
     * @param input
    ...
    /**
     * Takes the currentValue, transforms it into a Map, than uses the attributes from the delegate NoSqlSession to
     * update the values. The Map that is the result of that operation is transformed into byte[] and put under the
     * attributes key
     * 
     * @param currentValue
    ...
    /**
     * @return the value as it can be written to Redis. Note that if a key is not present, Redis will not overwrite the
     *         current value (i.e. will not delete it). Deleting should be a conscious action. This is why the
     *         attributes are written as one value. Control over them is exerted in
